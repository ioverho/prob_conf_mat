"""NOTE: THIS SCRIPT IS CALLED FROM `make docs-build`.

Taken from etl/docs: https://github.com/owid/etl/blob/master/docs/ignore/post-build/convert_notebooks.py
...
def convert_notebooks(docs_dir: Path, output_dir: Path, *, verbose: bool):
    ...
    if not output_dir.exists():
        raise ValueError(
            (
                ...
                "Please run this after building docs (e.g., after 'make docs.build'"
            ),
        )
    ...
    if not template_path.exists():
        raise ValueError(f"Error: Template page not found at {template_path}")
        return 1

    with template_path.open("r", encoding="utf-8") as f:
        default_zensical_template = f.read()
    ...
    for notebook_path in notebooks:
        try:
            ...
            notebook_title = nb.metadata.get(
                "title",
                relative_path.stem.replace("_", " ").title(),
            )
            ...
            template_path = html_path.parent / "index.html"
...
"""
    ...
    if relative_root != "./":
        ...
        template = template.replace(
            'href="javascripts/', f'href="{relative_root}javascripts/'
        )
        template = template.replace(
            'src="javascripts/', f'src="{relative_root}javascripts/'
        )
        ...
        template = template.replace(
            '"search":"assets/javascripts/',
            f'"search":"{relative_root}assets/javascripts/',
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog="convert_notebooks",
    )

    parser.add_argument("--docs_dir", type=str, default="./docs")
    parser.add_argument("--output_dir", type=str, default="./site")
