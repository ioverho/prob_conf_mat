...
class TestConfMatValidation:
    def test_dtype_conversion(self) -> None:
        ...
        validate_confusion_matrix(conf_mat=[[1, 0], [0, 1]])
        ...
        with pytest.raises(
            ...
            validate_confusion_matrix(conf_mat=[[1, "foo"], [0, 1]])
        ...
        with pytest.raises(
            ...
            validate_confusion_matrix(conf_mat=[[1.0, 0], [0, 1]])

        # uint should not fail
        validate_confusion_matrix(conf_mat=np.array([[1, 0], [0, 1]], dtype=np.uint))
        ...
        with pytest.raises(
            ...
            validate_confusion_matrix(
                conf_mat=np.array([[1, 0], [0, 1]], dtype=np.complex128)
            )

        # bool should not fail
        validate_confusion_matrix(conf_mat=np.array([[1, 0], [0, 1]], dtype=np.bool))

        with pytest.raises(
            ConfMatIOException,
            match="The loaded confusion matrix is not of type integer.",
        ):
            validate_confusion_matrix(conf_mat=np.array([[1, 0], [np.inf, 1]]))

    def test_shape(self) -> None:
        # 2D Square matrix should not fail
        conf_mat = np.ones((2, 2), dtype=np.int64)
        validate_confusion_matrix(conf_mat=conf_mat)
    ...
    def test_empty(self) -> None:
        with pytest.raises(ConfMatIOException, match="Some rows contain no entries"):
            validate_confusion_matrix(conf_mat=[[0, 0], [0, 1]])

        with pytest.warns(
            ConfMatIOWarning,
            match="Some columns contain no entries, meaning model never predicted it.",
        ):
            validate_confusion_matrix(conf_mat=[[0, 1], [0, 1]])
