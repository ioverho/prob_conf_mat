...
def distribution_plot(
    observed_values: typing.Dict[str, ExperimentResult],
    sampled_values: typing.Dict[str, ExperimentResult],
    ...
    total_num_experiments = sum(map(len, observed_values.values()))
    ...
    for group, group_values in observed_values.items():
        for i, experiment_values in enumerate(group_values):
            ordered_observed_values.append((group, i, experiment_values))

    all_min_x = []
    all_max_x = []
    all_max_height = []
    for i, (group, ii, point_value) in enumerate(ordered_observed_values):
        # Get summary statistics
        posterior_summary = summarize_posterior(
            sampled_values[group][ii].values, ci_probability=0.95
        )
        ...
        if plot_experiment_name:
            ...
            axes[i].set_ylabel(
                f"{group.name}/{point_value.experiment.name}",
                rotation=0,
                va="center",
                ha="right",
                fontsize=fontsize,
        ...
        if method not in IMPLEMENTED_METHODS:
            ...
            # Plot the kde
            sns.kdeplot(
                sampled_values[group][ii].values,
                fill=False,
                bw_adjust=bandwidth,
                ax=axes[i],
                color=edge_colour,
    ...
    grand_min_x = np.min(all_min_x)
    grand_max_x = np.max(all_max_x)

    # Decide on the xlim
    data_range = grand_min_x - grand_max_x
    metric_range = metric.bounds[1] - metric.bounds[0]

    # If the data range spans more than half the metric range
    # Just plot the whole metric range
    if (
        data_range / metric_range > 0.5
        and np.isfinite(metric.bounds[0])
        and np.isfinite(metric.bounds[1])
    ):
        x_lim_min = metric.bounds[0]
        x_lim_max = metric.bounds[1]
    else:
        if (grand_min_x - metric.bounds[0]) / metric_range < 0.05:
            x_lim_min = metric.bounds[0]
        else:
            x_lim_min = grand_min_x - 0.05 * (grand_max_x - grand_min_x)

        if (metric.bounds[1] - grand_max_x) / metric_range < 0.05:
            x_lim_max = metric.bounds[1]
        else:
    ...
    for i, (group, ii, point_value) in enumerate(ordered_observed_values):
        ...
        standard_length = (
            axes[i].transData.inverted().transform([0, 8])[1]
            - axes[i].transData.inverted().transform([0, 0])[1]
        )
