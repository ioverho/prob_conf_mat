...
class Study(Config):
    ...
    def __repr__(self):
        return f"self(experiments={self._list_experiments()}), metrics={self._metrics_store}"
    ...
    def report_aggregated_metric_summaries(
        ...
        for experiment_group in list(self.experiments.keys()):
            experiment_aggregation_result = self.get_metric_samples(
                metric=metric.name,
            ...
            distribution_summary = summarize_posterior(
                experiment_aggregation_result.values[:, class_label],
                ci_probability=self.ci_probability,
            )
    ...
    def plot_experiment_aggregation(
        ...
        aggregated_summary = summarize_posterior(
            agg_distribution_samples, ci_probability=self.ci_probability
        )
        ...
        for i, ax in enumerate(axes):
            if plot_base_line:
                ...
                ax.hlines(
                    0,
                    max(all_min_x[i], cur_xlim_max),
                    min(all_max_x[i], cur_xlim_min),
                    color=base_line_colour,
                    ls=base_line_format,
                    linewidth=base_line_width,
                    zorder=3,
    ...
    def plot_forest_plot(
        ...
        if figsize is None:
            ...
            _figsize = [None, None]
            _figsize[0] = 6.3
            _figsize[1] = max(0.28 * total_num_experiments, 2.5)
        else:
            _figsize = figsize
        ...
        for i, (experiment_group_name, experiment_group) in enumerate(
            ...
            for ii, (experiment_name, experiment) in enumerate(
                ...
                summary = summarize_posterior(
                    posterior_samples=samples, ci_probability=self.ci_probability
                )
            ...
            aggregation_result = self.get_metric_samples(
                metric=metric.name,
            ...
            axes[i].set_ylim(-1 if i == 0 else -0.5, ii + agg_offset + 1 + 0.5)
            axes[i].set_yticks(range(-1 if i == 0 else 0, ii + agg_offset + 2))
