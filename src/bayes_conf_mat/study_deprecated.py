...
from bayes_conf_mat.utils import seed_to_rng
...
class Study:
    def __init__(
        ...
        init_seed, self.rng = seed_to_rng(seed)  # type: ignore

        # Initialise the configuration back-end
        self.config = Config(
            name=name,
            seed=init_seed,
        ...
        self.experiment_groups: OrderedDict = OrderedDict()
        ...
        if self.cache_dir is not None:
            self.cache_dir = Path(self.cache_dir).resolve()  # types: ignore
